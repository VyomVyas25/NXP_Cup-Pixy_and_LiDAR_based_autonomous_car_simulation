...
class LineFollower(Node):
	...
	def __init__(self):
		...
		self.latest_speed = SPEED_MAX
    	self.latest_turn = TURN_MIN
	...
	def edge_vectors_callback(self, message):
		...
		if (vectors.vector_count == 1):    # 1 vector detected turn one side
			...
			if(abs(x)!=0 and abs(y)!=0):
				turn = math.atan2(y,x)*2.8*a1
				if(abs(y)<.2):
					turn=math.atan2(y,x)*a1/abs(y*abs(x)/4)
				speed = .8*(1-a1*abs(turn))
				if speed <=0.2:
					speed=0.2
			else:
				turn = 0
			...
			if abs(vectors.vector_1[1].y - vectors.vector_1[0].y) <= 45:
				...
				speed = 0.8*speed
				# print("sharp")
			#print(f"turn={turn}, x_diff={x},y_diff={y}")
		...
		if (vectors.vector_count == 2): # 2 vector detected -> magnitude equal
			...
			if(1.7<slope1<1.9 and slope2<.2):
				turn*=-10
			self.res = self.res/6000
			print(self.res)
			turn = (1-self.res)*turn + self.res
			#print(turn)
        ...
        if self.lidar_received:
           	self.rover_move_manual_mode()  # Publish the latest speed and turn values
        	self.lidar_received = False  # Reset the flag to wait for the next LIDAR message		self.rover_move_manual_mode(speed, turn)
